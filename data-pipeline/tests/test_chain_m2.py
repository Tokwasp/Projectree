...
from data_pipeline.storage import Node, NodeCandidate, TranscriptSegment
...
EXTRACTION_OUT = {
    ...
    "items": [
        {"id": "m1", "type": "DECISION", "predictedCategory": "BACKEND",
         ...
         "evidence": [{"segmentId": "seg-1", "quote": "소음 전처리는 CLOVA STT 전에 넣는 걸로 결정합시다."}]},
        {"id": "m2", "type": "ISSUE", "predictedCategory": "BACKEND",
         "title": "레디스 캐시 도입 여부 미확정", "content": "도입 여부 보류.",
         "evidence": [{"segmentId": "seg-2", "quote": "Redis 캐시 도입은 나중에 다시 논의해 봐야 할 것 같아요."}]},
    ],
}
...
def test_chain_persists_all_items_as_review_pending_candidates(session_factory, tmp_path):
    ...
    with session_factory() as s:
        ...
        segments = {
            row.segment_id: row
            for row in s.query(TranscriptSegment).all()
        }
        assert segments["seg-1"].raw_text == SEGMENTS[0]["text"]
        assert (
            segments["seg-1"].normalized_text
            == "소음 전처리는 CLOVA STT 전에 넣는 걸로 결정합시다."
        )
        assert segments["seg-1"].text == segments["seg-1"].normalized_text
        assert (
            segments["seg-1"].normalization_metadata[
                "dictionary_version"
            ]
            == "2026-07-30.3"
        )
    ...
    assert (
        res.lineage["extra"]["sttDictionaryVersion"]
        == "2026-07-30.3"
    )
    assert len(res.lineage["extra"]["sttDictionarySha256"]) == 64
