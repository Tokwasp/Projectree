...
against ``tests/fixtures/evaluation/candidate_quality/gold/*.json``. LLM output is never
pinned to exact strings in CI: this script is a diagnostic, not a unit test.

Usage:
    # score previously captured outputs
    python tests/tools/evaluation/evaluate_candidate_quality.py \
        --actual tests/fixtures/evaluation/candidate_quality/actual-20260801 \
        --out outputs/candidate-quality-eval/report.json

    # opt-in live run for every gold scenario (needs GMS_KEY etc.)
    python tests/tools/evaluation/evaluate_candidate_quality.py --live --profile candidate-quality-v1 ...
...
GOLD_DIR = ROOT / "tests" / "fixtures" / "evaluation" / "candidate_quality" / "gold"
