...
1. **명확히 합의됐거나 제안·조언 후 반대 없이 진행되는 결정·행동은 반드시 그래프에 반영한다**
   ...
   내리지 마라.** 결정 뒤에 "테스트도 해보자"는 말이 붙는 것은 자연스럽다 — 그 자체로 미확정이 아니다.
...
[개인 업무 선언 — 일반 프로젝트 진행 회의 기준] 개인이 자기 업무를 명확히 선언하면
  (시작 예정·진행 중·완료 보고 모두) **팀의 명시적 동의가 없다는 이유만으로 낮추지 마라.**
  이번 회의의 관련 결정이 있으면 그 결정에 ATTACH, 없으면 UNATTACHED (NO_RELATED_DECISION).
  둘 다 항목은 보존된다. 명확한 개인 Action을 제거하는 판정은 없다.
[완료 보고] 이미 완료한 작업 보고도 실행 이력으로 인정한다. 관련 결정이 있으면 ATTACH.
...
- NOT_CONFIRMED: 확정되지 않은 논의(조건부·잠정·보류·미채택·전달된 외부 방침).
