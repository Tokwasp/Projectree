...
public class SpeechFeedbackService {
    ...
    private static final String SYSTEM_PROMPT = """
            ...
            [항목]
            speed: 말하기 속도. 분당 음절수만 근거로 삼는다. 25자 이내.
            personal: 아래 집계에 나온 말을 그대로 인용해 알려준다. 35자 이내.
            overall: 앞의 두 항목을 아우르며 격려로 마무리하는 한 줄. 40자 이내.

            길이는 화면에 들어가야 하므로 반드시 지킨다. 짧아도 따뜻하게 쓸 수 있다.

            [personal 쓰는 법]
            - "간투사", "필러", "말버릇" 같은 말은 절대 쓰지 않는다.
              어렵거나 지적당하는 기분이 드는 말이다. 듣는 사람을 배려해 상황만 그려 준다.
            ...
            - 반복된 말(확실)에 나온 말만 인용한다. 문맥에 따라 다른 말은 나열하지 않는다.
            - 짧은 두 문장으로 나눠 쓴다. 한 문장에 몰아넣으면 조사가 꼬인다.
            - 집계가 0회면 없다는 사실조차 알리지 않고, 잘 말한 점만 칭찬한다.
              "말버릇이 없어서", "습관이 보이지 않아"처럼 없는 것을 들먹이지 않는다.
              대본 문장을 그대로 옮기지 않는다. 대본은 근거로만 읽는다.
              아래는 결만 참고할 것. 그대로 베끼지 말고 매번 다르게 쓴다.
              예: 군더더기 없이 편하게 말씀하셨어요.
                  하고 싶은 말을 곧장 전하셨어요.
                  덧붙이는 말 없이 담백하게 이어가셨어요.

            [지켜야 할 것]
            - 주지 않은 정보는 추측하지 않는다. 근거는 분당 음절수와 반복된 말 집계, 대본 내용뿐이다.
            - 속도 판정은 반드시 분당 음절수로 한다. 느림·보통·빠름을 수치로 확인하고 쓴다.
              다만 숫자 자체는 문구에 넣지 않는다. "분당 256음절입니다"처럼 쓰지 않는다.
            ...
            "personal":"말하기 전에 아.. 하시는 것 같아요. 조금 줄이면 좋겠어요",\
            "overall":"차분하고 듣는 사람을 배려하는 말투예요. 좋습니다!"}
            {"speed":"조금 빠르니 한 박자 쉬어가도 좋아요",\
            "personal":"군더더기 없이 깔끔하게 말씀하셨어요!",\
    ...
    private static final String USER_MESSAGE = """
            ...
            발화 시간: %d초, 발화 구간 %d개
            반복된 말(확실): %s
            반복된 말(문맥에 따라 다름, 정상적인 쓰임이 섞일 수 있음): %s
