const Starfield = () => {
  ...
  const config = {
    ...
    canvasColor: "var(--color-background)", // Canvas background color (only rgb)
  ...
  function setup(userConfig = {}) {
    ...
    for (let i = 0; i < config.numStars; i++) {
      ...
      const warmup = Math.floor(random(20, 150));
  ...
  class Star {
    ...
    reset() {
      ...
      this.baseSpeed = random(config.baseSpeed * 0.5, config.baseSpeed * 1.5);
      // this.vel.x = Math.cos(this.angle) * this.baseSpeed * 4;
      // this.vel.y = Math.sin(this.angle) * this.baseSpeed * 4;
    ...
    draw() {
      ...
      const alpha = Math.max(0.25, map(velMag, 0, 10, 0, 1));
      const weight = map(velMag, 0, 10, 1, 3);

      // ctx.lineWidth = weight;
      ctx.lineWidth = Math.max(1.8, weight);
  ...
  api.setup = setup;
  api.setAccelerate = setAccelerate;
  api.setOrigin = setOrigin;
  api.setOriginX = setOriginX;
  api.setOriginY = setOriginY;
  api.resize = resize;
  api.config = config;
  api.cleanup = cleanup;
